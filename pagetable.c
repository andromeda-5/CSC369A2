...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		pte->frame &= ~PG_DIRTY; //page no longer modified
		pte->frame |= PG_ONSWAP; //page evicted to swap
	}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	if (!(p->frame & PG_VALID)){
		...
		if (p->frame & PG_ONSWAP){
			...
			//fill frame by reading from swap
			int frame = allocate_frame(p);
			swap_pagein(frame, p->swap_off); //error check here?

			//shift frame number when storing
